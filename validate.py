...
# Those check for valid key format


    def calc_cs(self) -> int:
        gs = self.key.split('-')[:-1] # remove the last array (1234)
        return sum([sum(bytearray(g.encode())) for g in gs]) # total all character in bytecode

    def g1_valid(self) -> bool:
        g1 = self.key.split('-')[0] # Take the first array on key
        r = [(ord(v)<<i+1)%256^ord(v) for i, v in enumerate(g1[0:3])]
        # ord = change ascii character to number
        # x << y = move x to left by y bits (remember binary lesson)
        # % = remainder of division
        # i is the iteration count (start from  0) || v is the index value
        # g1[0:3] = get character from index 0 until character 3 (ABCDE -> ABC)
        if r != [221, 81, 145]:
            return False
        for v in g1[3:]:
            try:
